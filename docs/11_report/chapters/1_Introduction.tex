...
The purpose of the project is to provide a solution to the problem at hand. A real-time navigation system to assist users to their desired location within a museum. The significant part of this project is achieved by calculating the quickest route to the user's destination based from the user's current location, and displaying this route on the user's \remove{Android} device through the implementation of AR-assisted superimposition of navigational lines that the user would follow.
...
\note{good content here but just a very high-level overview is needed here, can you cut this down?}

This document is a report for the group project "Augmented Reality Navigation System for Commercial Spaces". This report fully describes the project undertaken \add{, containing eight main sections:} \remove{. However in order to control the length of the report, the reader is on occasion referred to a bibliographical reference if particular details are required in an area.}

\remove{The report contains eight main sections:}
